...
fnDeviceSettings = fullfile(dirname,'DeviceSettings.json');
res = readAdaptiveJson(fnAdaptive); 
devSettings = loadDeviceSettings(fnDeviceSettings);
...
if plotFFTsettings
    fftSettings = devSettings(1).tdData(1).sampleRate;
end
titleUse = strcat(outRec(1).tdData(1).chanFullStr,' stimRate-',sr,'Hz'); 
